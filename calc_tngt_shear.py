...
# Choose only lenses less than bin_max
        ...
        index = np.digitize(lens_radDist, binedges) - \
            1  # removed log10(rad_dist)

        for j in range(len(lens_id)):

            sin_phi2 = (-np.sin(lens_dec[j])*np.cos(dec[i]) + np.sin(dec[i])
                        * np.cos(lens_dec[j])*np.cos(lens_ra[j]-ra[i]))/theta_abs[j]
            cos_phi2 = np.sin(ra[i]-lens_ra[j]) * \
                np.cos(lens_dec[j])/theta_abs[j]
            ...
            den[index[j]] += lens_constant[j]*yij * \
                (1-lens_cdist[j]/cdist_mc[i])*R[i]
...
def calculate_dsigma_increments_vector(src, lenses, nnid, binedges):
    ...
    def _calculate_for_src(ra_s, dec_s, z_mean_s, cdist_mean_s, z_mc_s,
        ...
        # lens parameters
        ra_l, dec_l, z_l, const_l, cdist_l = nn_lens[[
            'ra', 'dec', 'zredmagic', 'const', 'cdist']].to_numpy().T

        # choose only the lenses behind the source TODO: apply min distance
        mask = (z_l < z_mean_s)
        ra_l, dec_l, z_l, const_l, cdist_l = \
            ra_l[mask], dec_l[mask], z_l[mask], const_l[mask], cdist_l[mask]
        ...
        # angular distance in between in radian
        theta_l = get_distance(ra_l, dec_l, ra_s, dec_s)
        ...
        # ra_l, dec_l, z_l, const_l, cdist_l, theta_l, rad_l, idx = \
        # ra_l[ mask ], dec_l[ mask ], z_l[ mask ], const_l[ mask ], cdist_l[ mask ],\
        # theta_l[ mask ], rad_l[ mask ], idx[ mask ]-1
        ...
        # a factor appearing in multiple times
        f1 = w_s * const_l * (1. - cdist_l / cdist_mean_s) * R_s
        # a factor apperaing in denominator
        f2 = const_l * (1. - cdist_l / cdist_mc_s) * R_s
        ...
        bstats = binned_statistic(
            rad_l,
            values=[
                f1 * e_tan,      # numerator for tangential part
    ...
    ra, dec, z_mean, cdist_mean, z_mc, cdist_mc, e1, e2, w = src[[
        'ra', 'dec', 'zmean_sof', 'cdist_mean', 'zmc_sof',
        'cdist_mc', 'e_1', 'e_2', 'weight'
    ]].to_numpy().T
    ...
    nbins = len(binedges) - 1
    (
        num_tan, num_crs, den_all,
        num_tan_alt, num_crs_alt
    ) = (
        np.zeros(nbins), np.zeros(nbins), np.zeros(nbins),
        np.zeros(nbins), np.zeros(nbins)
    )
    ...
    for s in tqdm(range(src.shape[0])):
        (
            num_tan_s, num_crs_s, den_all_s,
            num_tan_alt_s, num_crs_alt_s, npairs_s
        ) = _calculate_for_src(
            ra[s], dec[s], z_mean[s], cdist_mean[s],
    ...
    #                     ra, dec, z_mean, cdist_mean, z_mc,
    #                     cdist_mc, e1, e2, R, w, nnid
    #                 )
    #             )
    #         )
