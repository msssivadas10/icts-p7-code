...
def get_neighbour_indices(
        z: float, cat: Any, cm: ac.Cosmology, center_point: Any, b: float = 10.
) -> Any:
    r"""
    ...
    assert isinstance(
        cm, ac.Cosmology), '`cm` must be a astropy `Cosmology` object'
    assert np.ndim(center_point) == 2, '`center_point` must be a 2D array'
    assert np.size(
        center_point, 1) == 2, '`center_point` must have exactly 2 freatures'
...
def _test():
    ...
    cat = np.random.uniform(
        [-np.pi/2., 0.], [np.pi/2., 2*np.pi], size=[100, 2])
    ...
    def haversine_distance(x, y):
        ...
        return 2. * np.arcsin(
            np.sqrt(
                np.sin(0.5*(x[0] - y[0]))**2 + np.cos(x[0])
                * np.cos(y[0]) * np.sin(0.5*(x[1] - y[1]))**2
            )
        )
    ...
    for i in range(n_points):
        val = (i + 1) / (n_points+1)
        plt.plot(p[i, 1],      p[i, 0],      'o', ms=5,
                 color=(val, 0., 0.))  # test points
        plt.plot(cat[j[i], 1], cat[j[i], 0], 'o', ms=5,
                 color=(0., val, 0.))  # neighbors
