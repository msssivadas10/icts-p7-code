...
if(divide):
    ...
    np.savetxt("DES-regions.list",np.transpose([listramin,listramax,listdecmin,listdecmax]));
'''
    hdulist=pyfits.open("y3a2_gold2.2.1_redmagic_highdens_randoms.fits");
    data=hdulist[1].data
    rra=data.field("ra");
...
for ss in np.arange(sec.size):
    #ax=subplot(2,2,ss+1);
    listramin,listramax,listdecmin,listdecmax=np.loadtxt(sec[ss]+"-regions.list",unpack=1);
    for ii in range(listramin.size):
        ax.plot([listramin[ii],listramin[ii]],[listdecmin[ii],listdecmax[ii]],color='r');
        ax.plot([listramin[ii],listramax[ii]],[listdecmin[ii],listdecmin[ii]],color='r')
        ax.plot([listramin[ii],listramax[ii]],[listdecmax[ii],listdecmax[ii]],color='r')
        ax.plot([listramax[ii],listramax[ii]],[listdecmin[ii],listdecmax[ii]],color='r');

tight_layout();
savefig("Figure_chunk.pdf");
